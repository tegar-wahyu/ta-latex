...
keterkaitan \textit{single-hop} antar dua konsep di buku berbeda;
sistem tidak menyusun urutan multi-langkah lintas-disiplin, tidak
memodelkan tingkat kognitif Bloom, tidak mengintegrasikan keadaan
pembelajar (\textit{learner state}), dan tidak menghasilkan jalur
belajar terkomposisi. Dengan demikian, relasi berarah seperti
...
(Tier ekstraksi) beserta inferensi lintas-buku (Tier completion)---diilustrasikan
pada Gambar~\ref{fig:ontologi}.
...
panel pakar pada tiga aspek terpisah: (1) \textit{validitas}---apakah konsep
$A$ dan $B$ benar-benar berkaitan (Ya/Tidak); (2) \textit{ketepatan tipe}---
apakah label relasi yang diusulkan tepat (Benar/Salah); dan (3)
\textit{ketepatan arah}---apakah arah $A \to B$ sudah benar
(Benar/Terbalik/NA untuk relasi simetris). Relasi disajikan secara acak dan
\textit{score-blind}. Berbeda dengan Fase~1 yang melibatkan dua penilai
...
\begin{itemize}
  ...
  \item \textbf{Gwet's AC1 tidak berlaku.} Karena setiap relasi hanya memiliki
        ...
        antarpenilai tidak dapat dihitung untuk Fase~2.
\end{itemize}
