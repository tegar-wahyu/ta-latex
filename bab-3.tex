...
\begin{longtable}{@{}p{0.28\linewidth}p{0.65\linewidth}@{}}
  ...
  \textit{In-Context Learning} (ICL)
    & Strategi pemanfaatan LLM dengan menyertakan demonstrasi struktur
      ...
      \textit{fine-tuning}. \\
  \textit{Confidence} (LLM \textit{self-reported})
    & Skor dalam rentang $[0,1]$ yang dikeluarkan model klasifikasi
      LINTAS\_BUKU bersama setiap prediksi relasi (lihat skema respons
      pada Lampiran~\ref{lamp:prompt-completion}); diminta lewat
      petunjuk singkat dalam \textit{system prompt} ("\texttt{0.0--1.0;
      gunakan 0.85+ hanya jika sangat yakin}''). Dalam penelitian ini
      \textit{confidence} diperlakukan sebagai \textit{heuristik
      penyaring} yang tidak terkalibrasi secara formal: digunakan untuk
      \textit{gating} produksi (BERKAITAN\_DENGAN $\geq 0{,}85$, tipe
      lain $\geq 0{,}70$) dan untuk mengeluarkan kandidat dengan
      \textit{confidence} $< 0{,}5$ dari survei utama. Keandalannya
      divalidasi \textit{post hoc} pada
      Subbab~\ref{subsubsec:false-positive} mengingat literatur
      menunjukkan kalibrasi \textit{self-reported confidence} pada model
      bahasa cenderung terbatas \parencite{kadavath_language_2022}. \\
\end{longtable}
