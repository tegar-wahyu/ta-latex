...
\begin{enumerate}
  \item \textbf{Instansiasi (Purwarupa Sistem):} \textit{Dashboard}
        interaktif berbasis Neo4j untuk menyimpan, menelusuri, dan
        memvisualisasikan \textit{Knowledge Graph} kurikulum sains secara
        terintegrasi.
...
dinilai menggunakan skala empat poin yang telah didefinisikan pada
...
pada Subbab~\ref{sec:instrumen-evaluasi}). Data penilaian akhir dari para pakar menjadi
\textit{ground truth} untuk menghitung Fuzzy Precision, Recall, dan
F1-Score, serta menguji tingkat kesepakatan antarpenilai menggunakan
Gwet's AC1.
...
\begin{enumerate}
  \item \textbf{Pengukuran Reliabilitas Multi-Rater:} Skema evaluasi
        interkoneksi lintas-disiplin mengharuskan satu \textit{triple}
        divalidasi oleh dua pakar independen, yang memungkinkan
        perhitungan \textit{inter-rater agreement} menggunakan Gwet's
        AC1.
...
Mengacu pada definisi \textcite{studer_knowledge_1998} yang memperluas gagasan
\textcite{gruber_translation_1993}, ontologi dipahami sebagai spesifikasi formal dan eksplisit
atas sebuah konseptualisasi bersama terhadap suatu domain. Ontologi
dalam penelitian ini diposisikan sebagai model konseptual yang bersifat
...
\begin{longtable}{@{}p{0.30\linewidth}p{0.35\linewidth}p{0.27\linewidth}@{}}
  ...
  \bottomrule
  \endlastfoot
  Model LLM Ekstraksi \& Klasifikasi
    & Model bahasa besar untuk ekstraksi konsep/relasi dan klasifikasi
      relasi lintas-disiplin (KGC)
    & \texttt{gemini-2.5-flash-lite} (ekstraksi);
      \texttt{gemini-2.5-flash} (klasifikasi); akses Mei 2026 \\
  Strategi \textit{Prompt Engineering}
    & Variasi instruksi sistem untuk ekstraksi
    & \textit{Schema-constrained zero-shot} + \textit{context injection}
      + \textit{strict output constraints} \\
  Model \textit{Embedding}
    & Model representasi vektor untuk perhitungan similaritas
    & \texttt{text-embedding-004} (Gemini Embedding) \\
...
\begin{longtable}{@{}p{0.35\linewidth}p{0.37\linewidth}p{0.20\linewidth}@{}}
  ...
  \textit{Average Degree Centrality} (ADC)
    & Rata-rata derajat koneksi per node dalam graf
    & Rata-rata derajat ($\geq 0$) \\
  \textit{Graph Density}
    & Kepadatan koneksi relatif terhadap nilai teoritis maksimum
    & Nilai 0--1 \\
  \textit{Modularity}
    & Ukuran keterpisahan komunitas dalam graf
    & Nilai $-0{,}5$ hingga $1$ \\
  \textit{Description Completeness} (DC)
    & Proporsi entitas yang memiliki deskripsi tidak kosong
    & Nilai 0--1 \\
  \textit{Empty SubKonsep Rate} (ESR)
    & Proporsi entitas induk tanpa entitas turunan (dekomposisi gagal)
    & Nilai 0--1 \\
  \textit{Typed Relation Ratio} (TRR)
    & Rasio relasi bertipe spesifik terhadap seluruh relasi dalam KG
    & Nilai 0--1 \\
  \textit{Fuzzy Precision}
    & Akurasi tertimbang relasi yang diekstrak
      \parencite{harju_evaluating_2023}
    & Nilai 0--1 \\
  \textit{Fuzzy Recall}
    & Cakupan tertimbang relasi esensial yang berhasil ditemukan
    & Nilai 0--1 \\
  F1-Score
...
\begin{longtable}{@{}p{0.28\linewidth}p{0.65\linewidth}@{}}
  ...
  \textit{Embedding}
    & Representasi vektor numerik dari deskripsi konsep yang dihasilkan
      model \textit{embedding} (\texttt{text-embedding-004}, dimensi:
      3072). \\
...
Penelitian ini menggunakan dua jenis data yang saling melengkapi: (1)
data primer berupa dokumen kurikulum sebagai sumber ekstraksi
pengetahuan (Subbab~\ref{subsec:data-primer}), dan (2) data validasi
berupa penilaian pakar terhadap \textit{triple} relasi yang dihasilkan
sistem (Subbab~\ref{subsec:data-validasi}). Data primer dimanfaatkan
pada Tahap~1--4, sedangkan data validasi digunakan pada kedua fase
evaluasi untuk mengukur akurasi dan kelengkapan relasi yang diprediksi.

\subsection{Data Primer Kurikulum}

Sumber dan cakupan data primer telah dijabarkan pada
Subbab~\ref{subsec:data-primer}. Data ini berupa berkas PDF dari
repositori \url{https://buku.kemdikbud.go.id} untuk mata pelajaran
Fisika, Kimia, dan Biologi Kelas~XII Fase~F.

\subsection{Data Validasi Pakar}

Prosedur dan instrumen pengumpulan data validasi telah dijelaskan pada
Subbab~\ref{subsec:data-validasi}. Setiap \textit{triple} mendapatkan
penilaian dari dua pakar independen per mata pelajaran (total enam
\textit{reviewer}).
...
\begin{enumerate}
  ...
  \item \textit{Reviewer} masuk ke aplikasi, memilih mata pelajaran yang
        ditugaskan, dan menilai setiap \textit{triple} menggunakan skala
        empat poin (Subbab~\ref{subsec:validasi-manusia}).
...
\begin{enumerate}
  \item \textbf{Ekspor dan Agregasi:} Data penilaian diekspor dari Redis
        dalam format JSON per mata pelajaran. Setiap \textit{triple}
        memiliki dua rating yang diagregasi menjadi satu label konsensus
        berdasarkan kerangka evaluasi \textit{hybrid}.
  ...
  \item \textbf{Perhitungan Kesepakatan:} Gwet's AC1 dihitung untuk
        mengukur konsistensi \textit{inter-rater} sebelum konsolidasi,
        menggunakan label mentah dari masing-masing \textit{reviewer}.
...
Analisis struktural mengukur perubahan karakteristik topologi
\textit{Knowledge Graph} secara kuantitatif, khususnya untuk mengevaluasi
dampak proses KGC terhadap kualitas interkoneksi antarkonsep. Metrik
yang dihitung pada kondisi pre-KGC dan post-KGC meliputi ADC,
\textit{Graph Density}, \textit{Modularity}, TRR, dan DC, sebagaimana
telah didefinisikan pada Subbab~\ref{subsec:metrik-topologi},
\ref{subsec:metrik-ekstraksi}, dan~\ref{subsec:metrik-hubungan}. Seluruh nilai
diperoleh melalui kueri Cypher pada Neo4j Aura, sehingga bersifat
deterministik dan dapat direproduksi.

Hasil perbandingan pre-KGC dan post-KGC disajikan dalam bentuk tabel
berpasangan per mata pelajaran. Peningkatan ADC dan \textit{Density}
...
Analisis ini mengukur akurasi dan kelengkapan relasi yang dihasilkan KG.
Tingkat kesepakatan awal antarpakar diukur menggunakan Gwet's AC1,
kemudian kinerja sistem dievaluasi terhadap \textit{ground truth}
konsensus menggunakan Fuzzy Precision, Recall, dan F1-Score
\parencite{harju_evaluating_2023}, berdasarkan pemetaan nilai keanggotaan pada
