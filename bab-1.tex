...
tiga mata pelajaran terpisah secara administratif: Fisika, Kimia, dan Biologi.
Pemisahan ini berisiko menimbulkan fenomena \textit{siloed learning}
(Harvey \& Tree, 2025), kondisi di mana batas-batas disipliner yang kaku
...
memodelkan interkoneksi materi secara eksplisit. Ji dkk. (2022) mendefinisikan
...
data tersebut. Kim dan Shakya (2023) mengajukan pendekatan KG untuk
...
penerapan ini diperkuat oleh Abu-Salih dan Alotaibi (2024), yang melalui tinjauan
...
Meskipun menjanjikan, KG memiliki keterbatasan fundamental: sifatnya yang
...
otomatis (Ji dkk., 2022). Dalam konteks kurikulum sains, relasi lintas-disiplin
yang paling bernilai pedagogis justru seringkali bersifat implisit---tidak tersurat
dalam teks satu buku tunggal. Untuk mengatasi hal ini, tugas
\textit{Knowledge Graph Completion} (KGC) diterapkan untuk memprediksi
\textit{triple} yang hilang. Namun, metode KGC konvensional berbasis
\textit{embedding} struktural terbukti tidak efektif dalam skenario kurikulum
karena distribusi konsep yang tidak merata. Wei dkk. (2023) menegaskan bahwa
...
atas keterbatasan tersebut. Pan dkk. (2024) mengklasifikasikan pendekatan
...
\textit{triple} baru berdasarkan konteks. Yao dkk. (2023) mendemonstrasikan
bahwa dengan memperlakukan \textit{triple} pengetahuan sebagai sekuens teks,
LLM mampu memprediksi relasi semantik dengan akurasi melampaui metode
konvensional. Secara khusus untuk mengatasi kelangkaan informasi pada entitas
\textit{long-tail}, Wei dkk. (2023) membuktikan bahwa \textit{prompt few-shot}
pada tahap KGC---yaitu menyertakan beberapa contoh \textit{triple} valid sebagai
panduan di dalam \textit{prompt} klasifikasi---memungkinkan LLM menginferensikan
...
Namun, evaluasi pakar secara manual kerap menimbulkan ketidaksepakatan
antar-evaluator yang subjektif dan sulit diterapkan pada skala besar
(Gu dkk., 2024). Penelitian ini mengatasi masalah ini dengan mengadopsi
pendekatan \textit{LLM-as-a-judge}: model Gemini~3.1 Pro Preview berperan sebagai
...
dipertanggungjawabkan (Gu dkk., 2024).
...
\begin{enumerate}
  ...
  \item \textbf{Implementasi LLM-Assisted KGC:} Bagaimana efektivitas penerapan
        LLM-Assisted KGC---dengan \textit{zero-shot prompting} untuk ekstraksi
        entitas dan \textit{few-shot prompting} untuk klasifikasi relasi
        lintas-disiplin---dalam memprediksi relasi implisit antar-materi sains
  ...
  \item \textbf{Validasi Hybrid dan Evaluasi Metrik:} Bagaimana mengevaluasi
        ...
        kualitas KG secara kuantitatif menggunakan metrik
...
\begin{enumerate}
  ...
  \item Mengimplementasikan mekanisme LLM-Assisted \textit{Knowledge Graph
        Completion} menggunakan model Gemini~2.5 Flash untuk memprediksi dan
  ...
  \item Mengevaluasi kualitas \textit{knowledge graph} yang dihasilkan melalui
        ...
        \begin{enumerate}
          ...
          \item \textbf{Validasi Pakar:} memverifikasi akurasi ontologis dan
                ...
                \textit{Fuzzy Precision and Recall} (Harju \& Mesaros, 2023),
                pengujian kesepakatan Gwet's AC1, dan penengahan ketidaksepakatan
                menggunakan \textit{pipeline LLM-as-a-judge} berbasis
                Gemini~3.1 Pro Preview (Gu dkk., 2024).
        \end{enumerate}
\end{enumerate}
...
skripsi ini---mencakup skema ontologi OWL, strategi \textit{zero-shot prompting}
untuk ekstraksi, dan \textit{few-shot prompting} untuk klasifikasi relasi
lintas-disiplin---dapat diadaptasi untuk domain kurikulum lain atau jenjang
pendidikan yang berbeda. Kedua, KG kurikulum sains Fase~F berbahasa Indonesia
...
\begin{enumerate}
  ...
  \item \textbf{Model dan Strategi LLM:} Model Gemini~2.5 Flash
        (Google GenAI SDK) digunakan untuk ekstraksi entitas dan klasifikasi
        relasi lintas-disiplin, sedangkan Gemini~3.1 Pro Preview digunakan
        khusus sebagai \textit{LLM-as-a-judge} dalam \textit{pipeline} validasi.
        ...
        \textit{prompt engineering} dengan dua strategi berbeda:
        \textit{zero-shot schema-constrained prompting} untuk ekstraksi entitas,
        dan \textit{few-shot prompting} untuk klasifikasi relasi lintas-disiplin.
  ...
  \item \textbf{Infrastruktur Graf:} Graf diinstansiasi pada basis data Neo4j
        ...
        \textit{embedding} Gemini Embedding, \texttt{text-embedding-3-small},
        dan \texttt{multi\-lingual\--e5\--small}. Metrik topologi (ADC,
        \textit{Graph Density}, \textit{Modularity}) dihitung melalui kueri
        Cypher---tanpa pustaka \textit{Graph Data Science} (GDS)---agar
        kompatibel dengan tier Aura Free.
  ...
  \item \textbf{Cakupan Validasi:} Validasi pakar dilakukan oleh enam
        ...
        pada skala empat poin melalui aplikasi web KG Review App. Kesepakatan
