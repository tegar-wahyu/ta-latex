...
\begin{enumerate}
  ...
  \textit{few-shot} dan \textit{schema-constrained}. \textit{Prompt} menyertakan: nama bab dan
  prev/next chapter, daftar subchapter dari TOC, glosarium sebagai
  \textit{validation set}, materi pokok sebagai \textit{curriculum constraint}, konsep
  dari buku lain sebagai \textit{cross-book context}, serta beberapa contoh
  \textit{triple} sebagai demonstrasi (\textit{few-shot}).
  ...
  \textit{Prompt} mendefinisikan 15 tipe relasi intra-buku sesuai ontologi (Subbab~\ref{subsec:skema-ontologi}):
  ...
  DIKATALISIS\_OLEH, dan MEMILIKI\_SIFAT.
  ...
  \textit{Prompt} bersifat \textit{few-shot} (menyertakan contoh \textit{triple}
  sebagai demonstrasi) serta mengandalkan struktur instruksi dan penegakan skema
  untuk konsistensi \textit{output}.
...
\begin{enumerate}
  ...
  Tiap pasangan yang lolos \textit{pruning} diklasifikasikan oleh LLM (Gemini 2.5 Flash Lite)
