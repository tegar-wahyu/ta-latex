...
Secara kuantitatif, korpus mencakup total 708 halaman yang terbagi atas 17 bab
dan 77 subbab. Untuk diproses oleh \textit{pipeline}, teks dipecah menjadi 788
\textit{chunk} dengan ukuran 800 token dan tumpang-tindih (\textit{overlap}) 200
token antar-\textit{chunk}. Estimasi jumlah kata setelah dikurangi tumpang-tindih
adalah sekitar 110 ribu kata. Rincian ukuran korpus per mata pelajaran disajikan
pada Tabel~\ref{tab:ukuran-korpus}.
...
\begin{tabular}{@{}lrrrrr@{}}
  \toprule
  \textbf{Mata Pelajaran} & \textbf{Halaman} & \textbf{Bab} & \textbf{Subbab}
    & \textbf{\textit{Chunk}} & \textbf{Kata ($\approx$)} \\
  \midrule
  Fisika  & 228 & 9 & 39 & 252 & 35.600 \\
  Kimia   & 210 & 4 & 22 & 235 & 29.400 \\
  Biologi & 270 & 4 & 16 & 301 & 45.600 \\
  \midrule
  \textbf{Total} & \textbf{708} & \textbf{17} & \textbf{77} & \textbf{788}
    & \textbf{110.600} \\
  \bottomrule
\end{tabular}
\end{table}
...
F1-Score, serta menguji tingkat kesepakatan antarpakar pada Fase~1
...
\begin{enumerate}
  \item \textbf{Fase Evaluasi 1: Validasi Intra-Mapel \&
        ...
        kerja \textit{LLM-as-a-judge} (Gambar~\ref{fig:llm-judge}; selanjutnya
        disebut AI Judge) digunakan untuk mengevaluasi kumpulan \textit{triple} dalam satu
  ...
  \item \textbf{Fase Evaluasi 2: Validasi Inter-Mapel \& Diskusi
        ...
        melainkan melalui diskusi panel lintas-disiplin yang terdiri atas
        tiga pakar (satu wakil---Expert~2---per mata pelajaran)
...
Dengan demikian, relasi berarah seperti
\texttt{PRASYARAT\_UNTUK} pada Tier ekstraksi dimaknai sebagai
ketergantungan konseptual yang terdokumentasikan dalam materi sumber
(keputusan editorial penyusun buku), sedangkan relasi pada Tier
completion dimaknai sebagai sugesti per-pasangan yang dapat
dipertimbangkan secara lokal, bukan anjuran kronologis bagi siswa.
...
Metrik \textit{Fuzzy Precision}, \textit{Recall}, F1-Score, dan Gwet's AC1 pada
...
\textit{precision} (validitas) sebagai proporsi relasi yang dinilai valid,
serta \textit{ketepatan tipe} dan \textit{ketepatan arah} sebagai proporsi
relasi dengan tipe dan arah yang benar. Tiga hal membedakan perlakuan metrik
Fase~2 dari Fase~1:
...
\begin{itemize}
  \item \textbf{Tidak ada skala parsial.} Penilaian Fase~2 bersifat
        ...
        tidak digunakan.
  \item \textbf{\textit{Recall} dan F1 tidak dihitung.} Tidak tersedia himpunan
        rujukan menyeluruh atas seluruh relasi lintas-mapel yang
        \emph{seharusnya} ada (sebagai penyebut \textit{recall}); himpunan yang
...
\begin{enumerate}
  ...
  \item \textit{`Pakar} dapat menambahkan \textit{missing triples} dan
...
tinggi juga memperoleh nilai Precision tinggi dari pakar,
