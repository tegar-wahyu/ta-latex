...
Penelitian ini dikerjakan secara berkelompok oleh tiga peneliti (Yhoga,
Tegar, dan Soros) dengan pembagian tugas mengikuti tahapan
\textit{pipeline} KGC. Rincian pembagian tugas dan kontribusi
masing-masing anggota disajikan pada \ref{lamp:pembagian-tugas}.
...
Pemilihan ketiga mata pelajaran ini tidak dilakukan secara acak,
melainkan mengikuti struktur Kurikulum Merdeka itu sendiri. Pada Fase~E
(Kelas~X), Fisika, Kimia, dan Biologi masih dapat diajarkan secara
terpadu dalam satu mata pelajaran Ilmu Pengetahuan Alam (IPA), dan baru
dipisah menjadi tiga mata pelajaran yang berdiri sendiri pada Fase~F
(Kelas~XI dan XII) \parencite{kemendikbudristek_kepmen262_2022}. Hal ini
menunjukkan bahwa materi ketiganya berasal dari satu rumpun keilmuan yang
sama dan memiliki keterhubungan konseptual. Akan tetapi, ketika materi tersebut
terpecah menjadi tiga mata pelajaran terpisah pada Fase~F, keterhubungan
itu menjadi tidak lagi tampak secara eksplisit. Kondisi inilah yang
menjadikan Fase~F objek yang tepat untuk memetakan kembali interkoneksi
antarmateri yang menjadi fokus penelitian. Sebagai gambaran awal,
keterkaitan antarmateri semacam ini telah disinggung dalam literatur
\parencite{semilarski_expanding_2022}, misalnya pada hubungan konsep
Potensial Listrik (Fisika) dengan Potensial Elektrode (Kimia) atau
struktur kimia DNA (Kimia) dengan konsep Gen (Biologi). Pemetaan dan
verifikasi keterkaitan semacam inilah yang justru menjadi sasaran
penelitian ini.
...
\begin{enumerate}
  ...
  \item \textbf{Final consensus.} Berkas KG JSON awal, penilaian pakar,
        ...
        Meskipun model ekstraksi, \textit{completion}, dan
        \textit{LLM-as-a-judge} berasal dari keluarga Gemini, kesamaan ini
        tidak digunakan sebagai dasar klaim independensi evaluasi. Pilihan
        tersebut hanya menyeragamkan integrasi teknis, sedangkan independensi
        metodologis dijaga melalui pemisahan fungsi, data masukan, dan kondisi
        aktivasi model: Gemini~2.5 Flash Lite digunakan untuk ekstraksi,
        Gemini~2.5 Flash untuk klasifikasi relasi pada tahap
        \textit{completion}, sedangkan Gemini~3.1 Pro Preview dipilih sebagai
        \textit{judge} karena tahap ajudikasi membutuhkan kemampuan penalaran
        kontekstual yang lebih kuat atas masukan yang lebih banyak dan panjang.

        Untuk mengurangi risiko bias validasi diri sekaligus kerentanan bias
        penilaian LLM \parencite{gu_survey_2024}, \textit{triple} hasil
        ekstraksi relasi oleh LLM tidak langsung divalidasi oleh model yang
        menghasilkannya. Validasi utama tetap berasal dari dua pakar domain
        independen per mata pelajaran, sedangkan \textit{LLM-as-a-judge} hanya
        diaktifkan pada subset kasus yang tidak mencapai kesepakatan atau pada
        usulan \textit{missing triple} yang diajukan satu pakar. Pada setiap
        proses ajudikasi, \textit{prompt} judge dibangun ulang dari data
        validasi, bukan dari \textit{prompt} ekstraksi relasi: untuk perbedaan
        penilaian, masukan mencakup \textit{triple} yang diperselisihkan, dua
        label pakar, dan cuplikan teks PDF buku yang dipilih berdasarkan bab,
        mapel, relasi, target, dan deskripsi; untuk usulan \textit{missing
        triple}, masukan mencakup usulan pakar tunggal dan cuplikan teks PDF
        buku yang relevan. Dengan demikian, Gemini~3.1 Pro Preview tidak
        menjadikan hasil ekstraksi relasi sebagai rujukan kebenaran, melainkan
        menengahi ketidaksepakatan label terhadap bukti tekstual dari buku dan
        penilaian pakar.

        Secara metodologis, graf konsensus diperlakukan sebagai \textbf{graf
        audit}: seluruh relasi hasil sistem tetap disimpan bersama metadata
        validasi pakar, termasuk relasi dengan konsensus akhir \texttt{wrong}.
        Relasi semacam ini tidak diperlakukan sebagai pengetahuan final, tetapi
        dipertahankan untuk menjaga jejak keputusan pakar, memungkinkan
        reproduksi metrik atas graf konsensus tersimpan, analisis galat, dan uji ketahanan. Untuk pemanfaatan
        sebagai artefak pengetahuan tervalidasi, relasi berlabel \texttt{wrong}
        perlu dikeluarkan berdasarkan properti \texttt{consensus}/\texttt{status}
        atau metadata validasi yang menyertainya.
\end{enumerate}

% Gambar 3.2 ditampilkan persis setelah tahap "Final consensus" (tempat ia
% dirujuk) dan sebelum "KG Completion": enumerate dipecah, gambar [H] tampil
% penuh-lebar di antara kedua item, lalu enumerate dilanjutkan (penomoran lanjut).
\subfile{llm-judge}
...
\begin{enumerate}[resume]
  \item \textbf{KG Completion.} Masukan tahap ini adalah graf konsensus
        ...
        dan deskripsi singkat. Karena relasi diprediksi di antara konsep yang
        sudah ada dalam graf dan bukan digali dari teks mentah, tahap ini
        merupakan \textit{relation prediction}, yaitu salah satu subtugas KGC
        (lihat Subbab~\ref{subsec:kgc}).
        Pencarian kandidat ANN pada tahap ini bertumpu pada \textit{embedding}
        deskripsi \textit{node}, sehingga relasi berlabel \texttt{wrong} tidak
        memengaruhi kemiripan vektor secara langsung. Namun, karena classifier
        juga menerima konteks graf berupa relasi bertipe yang sudah ada,
        relasi \texttt{wrong} yang dipertahankan dalam graf audit tetap
        berpotensi muncul sebagai konteks klasifikasi; dampaknya dievaluasi pada
        Bab~5.
...
buku teks Kurikulum Merdeka (konsep, subbab, bab, beserta
keterkaitan semantik antarkonsep dalam satu buku), sejalan dengan
...
antardisiplin yang dapat dipertimbangkan, tetapi tidak menetapkannya
...
tidak menyusun jalur belajar multilangkah lintas-disiplin (batasan
...
\begin{enumerate}
  ...
  \item \textbf{Mendefinisikan properti (\textit{slot}) kelas.} Properti dibagi menjadi
...
benar-benar dimanfaatkan darinya. \emph{Pertama}, tipifikasi formal yang dapat
...
Kriteria keberhasilan yang ditetapkan sebelum evaluasi digunakan sebagai acuan
untuk menafsirkan hasil penelitian. Representasi interkoneksi kurikulum dianggap
tercapai apabila KG memuat setidaknya satu relasi lintas-disiplin tervalidasi
yang menghubungkan ketiga mata pelajaran. Untuk RQ2, penelitian bersifat
deskriptif-eksploratif sehingga tidak menetapkan ambang kuantitatif. Proses
\textit{completion} dianggap berhasil memperbaiki kualitas struktural KG apabila
AD meningkat dan \textit{Modularity} menurun pada kondisi post-KGC dibandingkan
pre-KGC. TRR diharapkan tidak menurun karena relasi baru seharusnya memperkaya
proporsi relasi bertipe eksplisit dalam graf, sedangkan DC diposisikan sebagai
kontrol kualitas deskripsi \textit{node}; metrik ini tidak harus meningkat
karena tahap \textit{completion} menambahkan \textit{edge}, bukan memperbarui
deskripsi \textit{node}. Dari sisi validitas, relasi lintas-buku dianggap
memenuhi kriteria apabila validitas pakar Fase~2 mencapai minimal 90\%.
Keandalan evaluasi intra-mapel dianggap memadai apabila Gwet's AC1 minimal
0{,}6, yaitu masuk kategori ``baik''. Kriteria AC1 ini hanya berlaku untuk
Fase~1 karena Fase~2 menggunakan keputusan konsensus panel, bukan dua penilaian
independen per relasi.
...
\begin{tabular}{@{}p{0.28\linewidth}p{0.65\linewidth}@{}}
  ...
  \textit{Confidence} (LLM)
    & Skor dalam rentang $[0,1]$ yang dikeluarkan model klasifikasi
      ...
      tingkat keyakinan model. \textit{Prompt} klasifikasi
      \emph{menginstruksikan} model untuk hanya memakai keyakinan tinggi
      ($\geq 0{,}85$ untuk \texttt{BERKAITAN\_DENGAN}; $\geq 0{,}70$ untuk
      tipe lain), tetapi instruksi ini bukan penyaring keras pada kode:
      setiap prediksi berlabel non-\texttt{NONE} tetap dituliskan ke graf
      beserta skornya. Penyaringan berbasis skor yang benar-benar diterapkan
      hanya mengecualikan kandidat berskor $< 0{,}5$ dari survei pakar (bukan
      dari graf). Karena skor ini bersifat \textit{self-reported} dan belum
      terkalibrasi, keandalannya diperiksa kembali pada
      Subbab~\ref{subsubsec:false-positive}. \\
\end{tabular}
\end{table}
...
\begin{enumerate}
  ...
  \item \textbf{Kalkulasi Metrik Struktural:} Metrik topologi graf
        (AD, \textit{Modularity}, TRR, DC) dihitung melalui kueri Cypher dan
        Neo4j Graph Data Science Plugin di Neo4j Aura pada kondisi pre-KGC dan
        post-KGC.
\end{enumerate}
...
representasi interkoneksi semantik dalam KG, karakteristik relasi implisit
lintas-disiplin, perubahan kualitas struktural graf, serta validitas relasi dan
keandalan evaluasinya.
...
tabel berpasangan. Peningkatan AD setelah KGC diinterpretasikan sebagai
indikator bahwa relasi implisit yang diprediksi LLM berhasil memperkaya
kepadatan jaringan pengetahuan. Penurunan \textit{Modularity} dibaca sebagai
indikasi berkurangnya keterpisahan komunitas dalam graf setelah relasi
lintas-disiplin ditambahkan. TRR digunakan untuk melihat sejauh mana relasi
dalam graf memiliki tipe hubungan yang eksplisit, sedangkan DC digunakan untuk
menilai kelengkapan deskripsi \textit{node}. Karena metrik struktural tidak
menilai kebenaran relasi, hasilnya tetap perlu dibaca bersama validasi pakar.
...
membandingkan nilai metrik graf (AD, \textit{Modularity}, TRR, DC) antara
kondisi pre-KGC dan post-KGC.
...
Pada relasi lintas-disiplin, analisis juga memperhatikan distribusi tipe relasi
dan pola keterhubungan antarmata pelajaran untuk menjawab karakteristik relasi
implisit yang ditemukan.
