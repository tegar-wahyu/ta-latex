...
Agar batas penilaian pakar tegas, cakupan validasi pakar dirinci sebagai
berikut, memperluas ringkasan pada ruang lingkup penelitian
(Subbab~\ref{sec:ruang-lingkup}).

\begin{itemize}
  \item \textbf{Objek yang divalidasi.} Pada Fase~1, objek penilaian adalah
        \textit{triple} relasi \emph{dalam-buku} (intra-mapel) per mata
        pelajaran, dengan lebih dari 100 \textit{triple} per mata pelajaran.
        Pada Fase~2, objek penilaian adalah relasi \emph{lintas-buku}
        (lintas-disiplin) hasil KGC. Hanya relasi kandidat dengan
        \textit{confidence} $\geq 0{,}5$ yang diikutkan dalam survei pakar;
        relasi berkeyakinan rendah (\textit{confidence} $< 0{,}5$) dikecualikan
        dari survei utama dan diperiksa terpisah secara kualitatif
        (Subbab~\ref{subsec:kualitatif}) karena skor keyakinan yang rendah
        menandai kandidat yang berpotensi tidak andal. Jumlah relasi yang
        dievaluasi dan tervalidasi dilaporkan pada
        Subbab~\ref{subsec:validasi-pakar}.
  \item \textbf{Aspek yang dinilai.} Fase~1 menilai \emph{validitas} relasi
        pada skala empat kategori (Subbab~\ref{subsec:validasi-manusia}).
        Fase~2 menilai tiga aspek terpisah---validitas, ketepatan tipe, dan
        ketepatan arah relasi---sebagaimana didefinisikan pada
        Subbab~\ref{subsec:metrik-fase2}. Pada kedua fase, pakar juga dapat
        mengusulkan \textit{missing triples} yang dianggap esensial.
  \item \textbf{Yang tidak divalidasi.} Pakar tidak menilai relasi
        struktural hierarkis (\texttt{HAS\_CHAPTER}, \texttt{HAS\_CONCEPT},
        dan sejenisnya), kelengkapan deskripsi \textit{node}, maupun skor
        \textit{confidence} LLM secara langsung. Validasi berfokus pada
        \emph{relasi} antarkonsep, bukan pada ekstraksi \textit{node} secara
        individual.
\end{itemize}

Adapun perlakuan terhadap \textit{triple} yang dinilai \emph{Salah} oleh
pakar---apakah dibuang atau tetap digunakan pada tahap penggabungan
(\textit{merge}) graf menuju Fase~2---berada di luar penetapan cakupan ini dan
diuraikan pada penjelasan metode penggabungan \textit{triple}.
% TODO(#77): tautkan \ref ke subbab metode penggabungan triple setelah isu #77 selesai.
...
pelajaran); profil keilmuan lengkap masing-masing pakar---latar belakang,
institusi, dan tahun---disajikan secara anonim pada
