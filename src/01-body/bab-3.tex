...
\begin{itemize}
  \item \textbf{Objek yang divalidasi.} Pada Fase~1, objek penilaian adalah
        ...
        (lintas-disiplin) hasil KGC. Dari 125 \textit{edge} keluaran
        \textit{pipeline} pada konfigurasi final, 8 \textit{edge} berkeyakinan
        rendah (\textit{confidence} $< 0{,}5$, seluruhnya
        \texttt{BERKAITAN\_DENGAN}) dikecualikan dari survei utama karena berada
        di bawah ambang keyakinan dan diperiksa terpisah sebagai dugaan
        \textit{false positive} (Subbab~\ref{subsec:kualitatif}); objek yang
        divalidasi pakar dengan demikian menyisakan 117 relasi
        (\textit{confidence} $\geq 0{,}5$).
