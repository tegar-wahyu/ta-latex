...
kerangka semantik yang mengatur struktur dan batas maknanya. \textcite{studer_knowledge_1998}, yang memperluas
...
kumpulan \textit{node} dan \textit{edge} tanpa makna pedagogis yang dapat diandalkan.
...
yang terhubung langsung ke \textit{node} $v$. Perlu dicatat bahwa metrik ini
adalah rata-rata derajat (\textit{average degree}, $\frac{2m}{n}$ untuk graf
tak-berarah), sehingga pembaginya adalah jumlah \textit{node} $|V|$; istilah ini
berbeda dari \textit{degree centrality} ternormalisasi ala Freeman yang membagi
derajat tiap \textit{node} dengan $|V|-1$. Dalam penelitian ini, ADC dihitung
