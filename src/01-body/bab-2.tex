...
Lebih rinci, \textcite{ji_survey_2022} membagi KGC ke dalam beberapa subtugas, di
antaranya \textit{link prediction}, \textit{entity prediction}, dan
\textit{relation prediction}, yaitu memprediksi relasi yang menghubungkan
pasangan entitas yang sudah ada di dalam graf. Subtugas \textit{relation
prediction} inilah yang menjadi fokus penelitian ini. Tugas ini perlu dibedakan
dari \textit{relation extraction} atau \textit{relation discovery} yang menggali
relasi langsung dari teks mentah. Pada KGC, relasi diprediksi di antara entitas
yang sudah terbentuk dalam graf, bukan dari potongan teks baru. Dengan demikian,
melengkapi relasi implisit antar-entitas yang sudah ada tetap tergolong KGC,
bukan sekadar penemuan relasi berbasis teks. Pendekatan inilah yang diadopsi
penelitian ini, sejalan dengan \textcite{abu-rasheed_llm-assisted_2025} yang
menerapkan LLM-Assisted KGC pada pemodelan kurikulum dan dibahas lebih lanjut
pada Subbab~\ref{sec:penelitian-terkait}.
...
subjektivitas masing-masing pakar \parencite{gu_survey_2024}. Untuk mengatasi ketidaksepakatan tersebut tanpa mengorbankan
...
manusia, LLM diposisikan secara spesifik sebagai penengah berbasis konteks
...
saat model melakukan ajudikasi---yakni menengahi dan memutus ketidaksepakatan
penilaian secara otomatis---proses penengahan oleh LLM menuntut
...
\textbf{Average Degree (AD)} mengukur rata-rata jumlah koneksi langsung yang
dimiliki setiap \textit{node} pada graf. Dalam penelitian ini, AD dihitung pada
subgraf konsep, sehingga rumusnya mengikuti definisi derajat rata-rata graf
\parencite{newman_networks_2010} dan digunakan sebagai statistik struktural KG
untuk membaca tingkat aktivitas atau keterhubungan graf
\parencite{ruenin_terekg_2024}:

\[
  AD = \frac{1}{|V_C|} \sum_{v \in V_C} \deg_C(v)
\]

\noindent dengan $V_C$ adalah himpunan \textit{node} \texttt{Concept} dan
$\deg_C(v)$ adalah jumlah relasi langsung dari \textit{node} konsep $v$ ke
\textit{node} konsep lain. Rumus ini perlu dibedakan dari \textit{degree
centrality} ternormalisasi, yang membagi derajat sebuah \textit{node} dengan
jumlah koneksi maksimum yang mungkin, yaitu $|V|-1$
\parencite{pavel_degree_2022}. Penelitian ini menggunakan rata-rata derajat tak
ternormalisasi karena tujuan metrik adalah membaca perubahan jumlah relasi
konseptual rata-rata per \texttt{Concept}, bukan proporsi koneksi maksimum yang
mungkin terbentuk. Dengan pembatasan pada subgraf konsep, AD mencerminkan
tingkat keterhubungan konseptual antarmateri sains tanpa dipengaruhi
\textit{node} struktural (kelas, bab, subbab). Metrik ini menjadi indikator
utama perubahan interkoneksi materi Fisika, Kimia, dan Biologi sebelum dan
sesudah KGC.

\subsubsection{Modularity}

\textbf{Modularity ($Q$)} mengukur seberapa kuat sebuah graf terbagi
menjadi komunitas-komunitas yang terpisah. Nilai $Q$ berkisar antara
$-0{,}5$ hingga $1$; nilai yang lebih tinggi menunjukkan pemisahan
komunitas yang lebih tegas, ditandai dengan koneksi yang rapat di dalam
komunitas dan jarang di luar komunitas. Nilai $Q = 0$ menunjukkan bahwa
pembagian komunitas yang dihasilkan tidak berbeda dari partisi acak.
Dalam konteks penelitian ini, arah perubahan yang diharapkan justru
\textbf{penurunan} $Q$ setelah KGC: nilai $Q$ yang makin rendah menandakan
komunitas antar-disiplin yang semula terpisah menjadi makin terhubung,
sejalan dengan tujuan mengintegrasikan materi lintas-mata-pelajaran.
...
buku (\textit{cross-book links}). Sejalan dengan AD antar-konsep yang
...
sebagai nama; entitas tersebut harus memiliki deskripsi agar dapat digunakan dalam
...
dinilai secara biner, yaitu benar (1,0) atau salah (0,0), yang
...
Untuk mengakomodasi hal tersebut, penelitian ini mengadopsi metrik
\textit{Fuzzy Precision} dan \textit{Fuzzy Recall}. Berdasarkan teori himpunan fuzzy, setiap elemen
...
sebagai titik tengah bagi relasi yang benar secara saintifik namun belum
sempurna, $0{,}25$ untuk derajat parsial yang lebih lemah, dan $0{,}0$ untuk
ketiadaan keanggotaan. Batasan setiap kategori beserta nilai keanggotaannya tidak
...
Untuk menghitung metrik evaluasi ini, penelitian ini mengadaptasi prinsip irisan
fuzzy (\textit{fuzzy intersection}) dari \textcite{harju_evaluating_2023}, yaitu
bahwa kecocokan antara prediksi model dan referensi pakar dapat dinyatakan
dengan nilai minimum antara keanggotaan prediksi dan keanggotaan referensi.
Dalam konteks penelitian ini, setiap \textit{triple} yang dihasilkan sistem
diperlakukan sebagai prediksi penuh, sedangkan tingkat kebenarannya ditentukan
oleh skor pakar (1{,}0; 0{,}5; 0{,}25; atau 0{,}0). Sebagai contoh, jika sistem
menghasilkan \textit{triple} (Fotosintesis, \texttt{MENGHASILKAN}, Glukosa),
maka dari sisi model prediksi tersebut dihitung sebagai satu keluaran penuh
($\hat{y}=1{,}0$). Namun, kontribusinya terhadap metrik mengikuti penilaian
pakar: bernilai 1{,}0 jika relasi dinilai benar, 0{,}5 jika benar tetapi
deskripsinya perlu diperbaiki, 0{,}25 jika maknanya masih kurang konteks, dan
0{,}0 jika relasi dinilai salah. Berbeda dari formulasi
soft-label umum pada \textcite{harju_evaluating_2023} yang menjumlahkan nilai
keanggotaan pada seluruh elemen referensi, rumus di bawah ini dioperasionalkan
untuk skenario validasi KG: penyebut \textit{Fuzzy Precision} adalah seluruh
relasi yang diekstraksi sistem, sedangkan penyebut \textit{Fuzzy Recall} adalah
relasi yang dinilai pakar ditambah \textit{missing triples} yang diusulkan pakar
sebagai relasi esensial. Dengan demikian, Fuzzy Precision, Fuzzy Recall, dan
F1-Score dalam penelitian ini digunakan sebagai metrik kinerja ekstraksi relasi,
bukan sebagai akurasi klasifikasi sederhana. Ketiganya dirumuskan sebagai
berikut:

\begin{enumerate}
  \item \textbf{Fuzzy Precision:} mengukur ketepatan relasi yang berhasil
        diekstrak oleh model LLM, yaitu seberapa besar bagian dari relasi hasil
        sistem yang diterima pakar sebagai benar, sebagian benar, atau kurang
        konteks.
        \[
          \text{Fuzzy Precision} =
          \frac{\text{Benar}
  ...
  \item \textbf{Fuzzy Recall:} mengukur kelengkapan ekstraksi terhadap referensi
        pakar. Dalam penelitian ini, referensi tersebut mencakup relasi yang
        berhasil diekstrak dan dinilai pakar, serta \textit{missing triples}
        yang diusulkan pakar sebagai relasi esensial tetapi belum dihasilkan
        sistem. Relasi hilang tersebut diperlakukan sebagai \textit{false
        negative}.
        \[
          \text{Fuzzy Recall} =
          \frac{\text{Benar}
  ...
  \item \textbf{F1-Score:} rata-rata harmonik antara Fuzzy Precision dan
        Fuzzy Recall
        untuk memberikan representasi kualitas ekstraksi model secara
        keseluruhan.
        \[
          F_1 = 2 \times
          \frac{\text{Fuzzy Precision} \times \text{Fuzzy Recall}}
               {\text{Fuzzy Precision} + \text{Fuzzy Recall}}
        \]
\end{enumerate}
...
\parencite{gwet_handbook_2014, james_counting_2026}.
...
yang dikodifikasi secara eksplisit dapat digunakan sebagai dasar
perencanaan urutan belajar. Perbedaan utama dengan penelitian
...
menjadi preseden penggunaan kombinasi pencarian kandidat dan ICL pada tugas KGC,
sementara penelitian ini mengadaptasi gagasan tersebut ke klasifikasi relasi
lintas-disiplin. Perbedaan kritis terletak pada konteks: dataset
...
Karya yang paling dekat dengan penelitian ini adalah
\textcite{abu-rasheed_llm-assisted_2025}, yang menerapkan LLM-Assisted KGC untuk
memodelkan kurikulum pendidikan tinggi. Pendekatan mereka memiliki banyak
kesamaan dengan penelitian ini, yakni ekstraksi konsep dari materi ajar dibantu
LLM dengan panduan ontologi, penautan topik antar-program studi melalui
kemiripan \textit{embedding}, validasi oleh pakar (\textit{human-in-the-loop}),
serta evaluasi gabungan berupa umpan balik pakar dan metrik kualitas graf.
Kesamaan ini sekaligus menegaskan bahwa pendekatan yang ditempuh penelitian ini
memang tergolong KGC, karena karya tersebut yang berciri serupa juga menyebut
prosesnya sebagai \textit{Knowledge Graph Completion}. Perbedaan utamanya
terletak pada konteks dan cakupan: \citeauthor{abu-rasheed_llm-assisted_2025}
bekerja pada jenjang pendidikan tinggi berbahasa Inggris dengan sumber berupa
materi kuliah (slide dan rekaman) serta menautkan modul antar-program studi,
sedangkan penelitian ini berfokus pada jenjang SMA Kurikulum Merdeka berbahasa
Indonesia dengan sumber berupa buku teks resmi serta menargetkan relasi lintas
tiga disiplin IPA (Fisika, Kimia, Biologi) dalam satu kurikulum nasional yang
sama.
...
dengan metrik \textit{Fuzzy Precision} dan \textit{Fuzzy Recall}. Kombinasi keempat elemen
