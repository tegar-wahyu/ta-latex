...
eksekusi. Kredensial koneksi Neo4j tidak disertakan. Seluruh kode lengkap,
mencakup kueri Cypher pada lampiran ini beserta \textit{notebook} \textit{pipeline}
ekstraksi, konstruksi, dan \textit{completion}, tersedia pada repositori publik
\url{https://github.com/tegar-wahyu/ta-latex/tree/main/assets/codes}.
...
Modularity dihitung menggunakan algoritma deteksi komunitas \textit{Louvain}
pada pustaka \textit{Graph Data Science} (GDS) Neo4j, yang mengidentifikasi
komunitas secara otomatis dan melaporkan nilai $Q$ beserta jumlah komunitas
(\textit{community count}). Graf diproyeksikan ke memori mencakup seluruh
\textit{node} dan relasi, lalu statistik Louvain dihitung untuk kondisi pre-KGC
dan post-KGC.

\begin{lstlisting}[style=cypherbox,caption={Proyeksi graf dan perhitungan modularity via GDS Louvain.},label={lst:cypher-modularity}]
// 1. Proyeksikan seluruh graf ke memori (semua node & relasi)
CALL gds.graph.project(
  'allGraph',   // nama graf in-memory
  '*',          // seluruh label node
  '*'           // seluruh tipe relasi
);

// 2. Hitung modularity dan jumlah komunitas (tanpa menulis ke graf)
CALL gds.louvain.stats('allGraph')
YIELD modularity, communityCount
RETURN modularity, communityCount;
\end{lstlisting}

Hasil kueri pada graf pre-KGC menghasilkan \textit{modularity}
0{,}8906276611017039 dengan 19 komunitas, sedangkan graf post-KGC menghasilkan
\textit{modularity} 0{,}8209188390248358 dengan 24 komunitas.

\subsubsection{Uji Ketahanan dengan Mengeluarkan Relasi \texttt{wrong}}

Untuk menghasilkan kolom ``\textit{Wrong} Dikeluarkan'' pada
Tabel~\ref{tab:struktural-robustness}, pola pencocokan relasi
\texttt{(:Concept)-[r]-(:Concept)} pada kueri AD diberi filter berikut. Node
\texttt{:Concept} tetap dipertahankan, sedangkan relasi berlabel
\texttt{wrong} dikeluarkan dari perhitungan.
