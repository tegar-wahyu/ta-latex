% ============================================================
%  LAMPIRAN 3 - TEMPLAT PROMPT
%  Compile standalone:  pdflatex lampiran-3.tex
...
dari kode implementasi final (\textit{notebook} \texttt{TA\_KG\_EXTRACTION.ipynb}
untuk Pass-1, \texttt{completion.py} untuk Pass-2, serta
\texttt{KG\_CONSENSUS.ipynb} untuk \textit{LLM-as-a-judge}).
Kunci API dan kredensial basis data yang ada pada
...
% ------------------------------------------------------------------
\subsection{Prompt Ekstraksi Pengetahuan (Pass-1)}
...
\texttt{gemini-2.5-flash-lite}. \textit{Prompt} disusun dari enam blok: (1)~instruksi
peran dan hierarki, (2)~konteks bab, (3)~daftar subbab, (4)~materi pokok dari
buku guru, (5)~glosarium, dan (6)~spesifikasi format
keluaran JSON yang ketat (\textit{schema-constrained}). Blok 3--5 hanya
disisipkan jika data terkait tersedia. Templat utama ditunjukkan pada
Kode~\ref{lst:prompt-ekstraksi}, sedangkan ketiga blok dinamis pada
...
OUTPUT FORMAT (STRICT JSON):
{
  ...
  "subtopics": [{
    ...
    "concepts": [{
      ...
      "relations": [{
        "type": "BAGIAN_DARI|MENYEBABKAN|BERGANTUNG_PADA|MENDEFINISIKAN|MEMPENGARUHI|BERINTERAKSI_DENGAN|MEMUNGKINKAN|MENGATUR|TERDIRI_DARI|BEREAKSI_DENGAN|MENGHASILKAN|TERLETAK_DI|DIFORMULASIKAN_SEBAGAI|DIKATALISIS_OLEH|MEMILIKI_SIFAT",
        "target": "...", "description": "..."
      }]
    }]
  }],
...
\begin{lstlisting}[style=promptbox,caption={Tiga blok dinamis yang disisipkan ke templat utama bila datanya tersedia.},label={lst:prompt-ekstraksi-blok}]
# Blok 3 - SUBCHAPTER
SUBCHAPTER:
...
% ------------------------------------------------------------------
\subsection{Prompt Klasifikasi Relasi Lintas-Buku (Pass-2)}
\label{lamp:prompt-completion}

Pada tahap \textit{completion}, setiap pasangan konsep kandidat (hasil pencarian
ANN dengan \texttt{COS\_THRESHOLD}~$=0{,}70$ dan \texttt{TOP\_K}~$=20$)
...
\noindent \textit{System prompt} \emph{meminta} model menetapkan keyakinan
tinggi (\texttt{LINTAS\_BUKU\_BERKAITAN\_DENGAN}~$\geq 0{,}85$; tipe lain
$\geq 0{,}70$; baris terakhir Kode~\ref{lst:prompt-sys}), namun ketentuan ini
merupakan \emph{instruksi ke model}, bukan penyaring keras pada kode. Pada
praktiknya, setiap prediksi berlabel non-\texttt{NONE} dengan arah yang
konsisten terhadap sifat simetri tipenya dituliskan ke Neo4j beserta skornya;
penyaringan berbasis skor yang benar-benar diterapkan hanya mengecualikan
kandidat ber-\texttt{confidence}~$< 0{,}5$ dari survei pakar Fase~2 (bukan dari
graf). Akibatnya, seluruh 21 \textit{edge}
\texttt{LINTAS\_BUKU\_BERKAITAN\_DENGAN} yang tersimpan
ber-\texttt{confidence}~$< 0{,}85$ (dianalisis pada
Subbab~\ref{subsubsec:false-positive}).

\noindent Prediksi non-\texttt{NONE} kemudian dituliskan ke
Neo4j sebagai \textit{edge} berarah dengan subset properti yang
dipertahankan (\texttt{relation\_type}, \texttt{confidence},
\texttt{description}, dan \texttt{method}); \texttt{evidence\_quote} dan
\texttt{rationale} pada skema respons digunakan saat klasifikasi namun
tidak ikut dipersistensi. Kode~\ref{lst:edge-contoh} menampilkan satu
rekaman \textit{edge} lintas-buku aktual (LB001) sebagaimana tersimpan,
memperlihatkan asal nilai \textit{confidence} yang dianalisis pada
Bab~5.

\begin{lstlisting}[style=promptbox,caption={Contoh satu rekaman \textit{edge} lintas-buku aktual (LB001) sebagaimana dipersistensi ke Neo4j. Nilai \texttt{confidence} adalah skor \textit{self-reported} model untuk relasi ini (lihat Tabel~\ref{tab:conf-kalibrasi}).},label={lst:edge-contoh}]
{
  "rel_type":     "LINTAS_BUKU_PRASYARAT_UNTUK",
  "source_name":  "DNA (Asam 2-Deoksiribonukleat)",   // Kimia Kelas XII
  "target_name":  "Gen",                                // Biologi Kelas XII
  "properties": {
    "relation_type": "PRASYARAT_UNTUK",
    "confidence":    1.0,
    "description":   "Pemahaman tentang struktur kimia DNA adalah
                      prasyarat untuk memahami gen sebagai segmen
                      fungsional dari DNA.",
...
% ------------------------------------------------------------------
\subsection{Prompt LLM-as-a-Judge (Konsensus Fase~1)}
