...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[
    ...
    \node[layer, fill=blue!12, minimum width=10.6cm, minimum height=6.6cm] (b70) {};
    \node[title, anchor=north] at ([yshift=-7pt]b70.north)
      {0{,}70 / $k$=20 \textemdash{} 117 relasi};

    % Mid-1 (τ=0,75)
    \node[layer, fill=blue!28, minimum width=8.2cm, minimum height=4.9cm] (b75) {};
    \node[title, anchor=north] at ([yshift=-7pt]b75.north)
      {0{,}75 / $k$=15 \textemdash{} 72 relasi};

    % Mid-2 (τ=0,80)
    \node[layer, fill=blue!55, minimum width=5.8cm, minimum height=3.2cm] (b80) {};
    \node[titlew, anchor=north] at ([yshift=-7pt]b80.north)
      {0{,}80 / $k$=10 \textemdash{} 42 relasi};

    % Inner (τ=0,85) — darkest
    \node[layer, fill=blue!80, minimum width=3.6cm, minimum height=1.5cm] (b85) {};
    \node[titlew, anchor=center] at (b85)
      {0{,}85 / $k$=5 \textemdash{} 14 relasi};
  \end{tikzpicture}

  \vspace{0.6em}
  {\footnotesize\itshape\raggedright
    Contoh relasi representatif per konfigurasi:\par}
  {\footnotesize\raggedright
    0{,}70/$k$=20: Arus Listrik $\to$ \texttt{APLIKASI\_DARI} $\to$ Elektroforesis;\quad
    0{,}75/$k$=15: Defleksi Gerak Muatan $\to$ \texttt{PRASYARAT\_UNTUK} $\to$ Elektroforesis;\quad
    0{,}80/$k$=10: Potensial Elektrode $\to$ \texttt{APLIKASI\_DARI} $\to$ Potensial Listrik;\quad
    0{,}85/$k$=5: DNA $\leftrightarrow$ \texttt{SAMA\_DENGAN} $\leftrightarrow$ DNA.\par}
