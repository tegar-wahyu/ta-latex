...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[
    ...
    titlew/.style = {font=\small\bfseries, text=white},
    contoh/.style = {font=\scriptsize\itshape, text=black!85},
    contohw/.style = {font=\scriptsize\itshape, text=white},
  ]
    % Outer (τ=0,70) — palest blue
    \node[layer, fill=blue!12, minimum width=11.2cm, minimum height=7.6cm] (b70) {};
    \node[title, anchor=north] at ([yshift=-6pt]b70.north)
      {0{,}70 / $k$=20 \textemdash{} 117 relasi};
    \node[contoh, anchor=north] at ([yshift=-22pt]b70.north)
      {Contoh: Arus Listrik $\to$[\textsc{aplikasi\_dari}]$\to$ Elektroforesis};

    % Mid-1 (τ=0,75)
    \node[layer, fill=blue!28, minimum width=8.6cm, minimum height=5.4cm] (b75) {};
    \node[title, anchor=north] at ([yshift=-6pt]b75.north)
      {0{,}75 / $k$=15 \textemdash{} 72 relasi};
    \node[contoh, anchor=north] at ([yshift=-22pt]b75.north)
      {Contoh: Defleksi Gerak Muatan $\to$[\textsc{prasyarat\_untuk}]$\to$ Elektroforesis};

    % Mid-2 (τ=0,80)
    \node[layer, fill=blue!55, minimum width=6.2cm, minimum height=3.4cm] (b80) {};
    \node[titlew, anchor=north] at ([yshift=-6pt]b80.north)
      {0{,}80 / $k$=10 \textemdash{} 42 relasi};
    \node[contohw, anchor=north, text width=5.8cm] at ([yshift=-22pt]b80.north)
      {Contoh: Potensial Elektrode $\to$[\textsc{aplikasi\_dari}]$\to$ Potensial Listrik};

    % Inner (τ=0,85) — darkest
    \node[layer, fill=blue!80, minimum width=3.8cm, minimum height=1.6cm] (b85) {};
    \node[titlew, anchor=north] at ([yshift=-4pt]b85.north)
      {0{,}85 / $k$=5 \textemdash{} 14 relasi};
    \node[contohw, anchor=north, text width=3.4cm] at ([yshift=-18pt]b85.north)
      {Contoh: DNA $\leftrightarrow$[\textsc{sama\_dengan}]$\leftrightarrow$ DNA};
  \end{tikzpicture}
