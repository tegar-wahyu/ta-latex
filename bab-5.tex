...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}
    \begin{axis}[
      ...
      title={\textbf{\textit{Parameter Sweep}: Pengaruh \textit{Threshold} dan Top-$k$}\\
             \textbf{terhadap Jumlah Relasi Lintas-Buku}},
      ...
      ymin=0, ymax=150,
      ylabel={Jumlah \textit{edge}},
      xlabel={Konfigurasi (\textit{threshold}, top-$k$)},
      ylabel style={font=\small},
      xlabel style={font=\small},
      % Legend spread horizontally across the top, inside the plot (as in source)
      legend style={
        at={(0.5,0.97)}, anchor=north,
        font=\scriptsize, fill=white, fill opacity=0.9,
      ...
      % Descending total line, centered over each group (bar shift=0pt).
      % Declared first so "Total" heads the legend, as in the source figure.
      \addplot[
        mark=*, dashed, black, thick, bar shift=0pt,
        mark options={fill=black, scale=1.1},
        nodes near coords,
        nodes near coords style={font=\scriptsize\bfseries, anchor=south, yshift=3pt},
      ] coordinates {(A,125) (B,72) (C,42) (D,14)};
      \addlegendentry{Total}

      % Bars at explicit shifts so the 5 categories stay centered as a group.
      \addplot[fill=orange!75, draw=orange!90, bar shift=-12pt] coordinates {(A,15) (B,10) (C,9)  (D,3)}; \addlegendentry{APLIKASI\_DARI}
      \addplot[fill=purple!55, draw=purple!75, bar shift=-6pt]  coordinates {(A,21) (B,5)  (C,2)  (D,0)}; \addlegendentry{BERKAITAN\_DENGAN}
      \addplot[fill=green!60,  draw=green!75,  bar shift=0pt]   coordinates {(A,31) (B,20) (C,12) (D,4)}; \addlegendentry{MEMPERDALAM}
      \addplot[fill=cyan!65,   draw=cyan!80,   bar shift=6pt]   coordinates {(A,57) (B,36) (C,18) (D,6)}; \addlegendentry{PRASYARAT\_UNTUK}
      \addplot[fill=red!65,    draw=red!80,    bar shift=12pt]  coordinates {(A,1)  (B,1)  (C,1)  (D,1)}; \addlegendentry{SAMA\_DENGAN}
    \end{axis}
  \end{tikzpicture}
...
\begin{figure}[htbp]
  ...
  \begin{minipage}[c]{0.46\linewidth}
    ...
    \begin{tabular}{@{}p{1.95cm}@{\,}p{1.55cm}@{\,}p{1.65cm}@{\,}c@{}}
      ...
      BERKAITAN\_DENGAN  & Fotoelektron (Fis)   & Pot. Elektrode (Kim) & 0{,}7 \\
...
\begin{itemize}
  ...
  \item Fosforilasi Oksidatif [Biologi] $\to$ Oksidasi [Kimia]:
    \texttt{PRASYARAT\_UNTUK} (conf = 1{,}0). Pemahaman reaksi oksidasi secara kimia
