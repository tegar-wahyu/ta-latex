...
\begin{enumerate}
  ...
  adjudikasi \textit{LLM-as-a-judge} per mata pelajaran diberikan pada
...
Validitas dan ketepatan tipe mencapai 100\% pada keempat subset,
mengonfirmasi bahwa pengetatan ambang memang menyaring sisa
\textit{noise} validitas pada $\tau$=0{,}70. Namun, ketepatan arah relasi
tetap berada di rentang 71--86\% lintas-konfigurasi tanpa
kecenderungan monotonik terhadap pengetatan parameter, menegaskan
bahwa kesalahan arah bukan persoalan ambang --- relasi terbalik
...
\caption{Hasil validasi pakar atas relasi lintas-buku per konfigurasi parameter.
  Kolom $n$ menyatakan jumlah \textit{edge} yang lolos rekualifikasi atas
  \textit{embedding} mutakhir; rinciannya dijelaskan pada catatan kaki di bawah.}
...
\midrule
\endhead
0{,}70 / $k$=20 (paling permisif) & 125 & 101 & 100{,}0\% (101/101) & 100{,}0\% (101/101) & 75{,}2\% (76/101) \\
...
\noindent\textit{Catatan.} Survei Fase~2 menilai 117 \textit{edge} hasil
konfigurasi 0{,}70/$k$=20 produksi. Rekualifikasi atas \textit{embedding}
mutakhir hanya mereproduksi 101 dari 117 \textit{edge} tersebut pada
ambang $\tau\!\ge\!0{,}70 \wedge \text{rank}\!\le\!20$ (16 \textit{edge}
tidak lagi lolos akibat \textit{drift} skor kemiripan); kolom $n$ pada
Tabel~\ref{tab:validasi-sweep} mengacu pada subset yang tereplikasi ini.
Angka agregat $n=117$ pada Subbab~\ref{subsec:validasi-pakar} tetap
sahih sebagai laporan validasi produksi.
