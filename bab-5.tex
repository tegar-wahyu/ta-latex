...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=\linewidth]{images/media/image3.png}
...
\caption{Hasil validasi pakar atas relasi lintas-buku per konfigurasi parameter.
  ...
  \textit{sweep} produksi; kolom $n$ adalah jumlah \textit{edge} yang
...
\subsubsection{Pemilihan Konfigurasi Produksi}\label{subsubsec:konfigurasi-final}

Berdasarkan Tabel~\ref{tab:validasi-sweep}, \textbf{konfigurasi produksi
yang dipilih untuk \textit{Knowledge Graph} final adalah
...
\begin{enumerate}
  ...
  \item \textbf{\textit{Noise} sisa pada $\tau=0{,}70$ bersifat terbatas
    ...
    dan dapat dihapus secara terarah dari graf produksi tanpa
    mempengaruhi 114 \textit{edge} lainnya.

  \item \textbf{Pengetatan \textit{threshold} tidak menyelesaikan sumber
    kesalahan dominan.} Sebagaimana ditunjukkan oleh
    Tabel~\ref{tab:validasi-sweep} dan, lebih kuantitatif, oleh
    Tabel~\ref{tab:conf-kalibrasi} pada Subbab~\ref{subsubsec:false-positive},
    ketepatan arah tetap stagnan pada rentang 71--86\% lintas-konfigurasi
    tanpa kecenderungan monotonik. Dengan demikian \textit{tradeoff}
    cakupan-versus-presisi-arah menjadi negatif: stricter configurations
    kehilangan cakupan tanpa imbal balik perbaikan arah yang merupakan
    sumber kesalahan dominan.
\end{enumerate}
...
dapat menyaring graf produksi menggunakan ambang \textit{cosine}
...
Perlu dicatat bahwa asumsi $\hat{y}_i = 1{,}0$ (``keyakinan penuh
model'') pada Subbab~\ref{subsec:validasi-manusia} merupakan
penyederhanaan aljabar khusus untuk rumus \textit{Fuzzy Precision}
\parencite{harju_evaluating_2023}, di mana skor keanggotaan LLM
diabaikan sehingga irisan \textit{fuzzy} mengikuti penilaian pakar.
Di sini sebaliknya, variansi \textit{confidence} aktual yang
dikeluarkan LLM dianalisis sebagai sinyal pemisah relasi
\textit{fallback} dari relasi spesifik --- konsisten dengan
peran \textit{confidence} sebagai heuristik penyaring (lihat
Tabel~\ref{tab:definisi} dan Lampiran~\ref{lamp:prompt-completion}),
...
$[0{,}7;\;0{,}85)$, sehingga ambang gerbang produksi
