...
\begin{figure}[htbp]
  ...
  % Concept di atas (banyak relasi di sekitarnya), Grade/Subject di bawah.
  \node[class] (concept) {Concept};
  \node[class, below=1.2cm of concept]  (subtopic) {Subtopic};
  \node[class, below=1.2cm of subtopic] (chapter)  {Chapter};
  \node[class, below=1.2cm of chapter]  (grade)    {Grade/\\Subject};

  \node[target, above=2cm of concept] (target) {ConceptTarget\\(Placeholder)};

  % Relasi Hierarki (label di kanan tulang punggung)
  ...
  % --- 4. RELASI SEMANTIK (WITHIN-BOOK) — Concept ke Target (di atas) ---
  ...
  \pgfresetboundingbox
  \useasboundingbox
    ([xshift=-3mm,yshift=-3mm]xblabel.west |- grade.south)
    rectangle
    ([xshift=3mm,yshift=3mm]d_bool.east |- target.north);
