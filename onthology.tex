...
\begin{figure}[htbp]
  \centering
  \adjustbox{max width=\linewidth, max totalheight=0.9\textheight, center}{%
  \begin{tikzpicture}[
  ...
  % Loop kecil standar; label diletakkan sebagai node independen agar
  % bounding box tidak meledak akibat geometri loop.
  \draw[cross] (concept) edge[loop left, looseness=5] (concept);
  \node[below left=0.4cm and 0.2cm of concept, fill=white, inner sep=2pt,
        font=\tiny, align=left, draw=purple!30, rounded corners] {
  ...
  \pgfresetboundingbox
  \useasboundingbox
    ([xshift=-3mm,yshift=-3mm]target.west |- grade.south)
    rectangle
    ([xshift=3mm,yshift=3mm]grade.east |- target.north);
