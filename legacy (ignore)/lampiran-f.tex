% ============================================================
%  LAMPIRAN F – KUERI CYPHER
%  Compile standalone:  pdflatex lampiran-f.tex
...
% ==================================================================
\section{Konstruksi Graf (Strategi \texttt{MERGE})}
\label{lamp:cypher-konstruksi}

\subsection{Batasan Keunikan (\textit{Constraints})}
...
\subsection{Pass~1 --- Node Struktural}
...
\subsection{Pass~2 --- Relasi Intra-Buku}
...
\subsection{Pass~3 --- Relasi Lintas-Buku}

Pass~3 menulis \textit{edge} lintas-buku berprefiks \texttt{LINTAS\_BUKU\_}
dengan \texttt{graph\_role = 'semantic'}, sehingga dapat dibedakan dari relasi
struktural maupun intra-buku saat analisis pre/post-KGC.

\begin{lstlisting}[style=cypherbox,caption={Penulisan edge lintas-buku (Pass~3).},label={lst:cypher-pass3}]
...
% ==================================================================
\section{Kueri Perhitungan Metrik}
\label{lamp:cypher-metrik}

Kueri berikut menghitung metrik yang didefinisikan pada Bab~2. Metrik
struktural utama (ADC dan \textit{Graph Density}) dihitung pada
\textbf{subgraf konsep} --- hanya \textit{node} \texttt{:Concept} dan relasi
langsung antar-konsep --- agar tidak terpengaruh \textit{node} struktural.

\subsection{Average Degree Centrality (ADC)}

\begin{lstlisting}[style=cypherbox,caption={ADC pada subgraf konsep.},label={lst:cypher-adc}]
MATCH (m:Concept)
...
\begin{lstlisting}[style=cypherbox,caption={Graph Density pada subgraf konsep. Untuk kondisi pre-KGC tambahkan filter \texttt{WHERE NOT type(r) STARTS WITH 'LINTAS\_BUKU'}.},label={lst:cypher-density}]
MATCH (n:Concept)
...
\subsection{Description Completeness (DC)}
...
\subsection{Empty SubKonsep Rate (ESR)}
...
\subsection{Typed Relation Ratio (TRR)}
