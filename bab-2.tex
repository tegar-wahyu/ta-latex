...
rata-rata jumlah koneksi yang dimiliki oleh setiap \textit{node}
...
\noindent dengan $|V|$ adalah jumlah \textit{node} dalam graf dan $\deg(v)$
adalah jumlah relasi yang terhubung langsung ke \textit{node} $v$.

Dalam penelitian ini, ADC dihitung dalam dua bentuk. ADC \textbf{graf
keseluruhan} menghitung seluruh \textit{node} dan relasi dalam basis data graf,
termasuk \textit{node} struktural seperti kelas, bab, subbab, konsep, serta
target konsep. ADC \textbf{antar-konsep} hanya melibatkan \textit{node} konsep
dan relasi langsung antar-konsep, merepresentasikan tingkat
keterhubungan konseptual antarmateri sains tanpa dipengaruhi oleh \textit{node}
